    ...
    for doi in all_dois:
        ...
        for env_name, summaries in all_summaries:
            ...
            if env_name not in final_summary:
                ...
                # add test report(s) to final summary
                if duplicates := set(final_summary[env_name].keys()) & set(summary.keys()):
                    warnings.warn(f"Overwriting duplicate tests {duplicates} for doi {doi} in test env {env_name}")

                final_summary[env_name].update(summary)
